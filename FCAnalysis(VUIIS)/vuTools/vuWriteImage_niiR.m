function vuWriteImage_niiR(X,filename,varargin)
...
p.FunctionName='vuWriteImage_nii';
p.parse(varargin{:});
...
function writeNifti(IM,fullFilename)
...
if (isfield(IM,'Parms')&&isfield(IM.Parms,'srow_x')&&isfield(IM.Parms,'aux_file')&&isfield(IM.Parms,'scl_slope'))
    ...
    niftiHeader.bvalue = IM.b_value; %bvalue
    ...
    niftiHeader.qform_code = 0; %qform code ***
    niftiHeader.sform_code = 0; %sform code ***
    niftiHeader.quartern_b = 0; %Quarternion b param ***
    niftiHeader.quartern_c = 0; %Quarternion c param ***
    niftiHeader.quartern_d = 0; %Quarternion d param ***
    ...
    if (isfield(IM,'Orientation'))
        srow_x = [IM.Orientation(1,:) IM.Origin(1)];
        srow_y = [IM.Orientation(2,:) IM.Origin(2)];
        try
            srow_z = [IM.Orientation(3,:) IM.Origin(3)];
        catch
            srow_z = [IM.Orientation(3,:) 0];
        end
    else
        srow_x = [1 0 0 IM.Origin(1)];
        srow_y = [0 1 0 IM.Origin(2)];
        try
            srow_z = [0 0 1 IM.Origin(3)];
        catch
            srow_z = [0 0 1 0];
        end
    end
...
if isfield(niftiHeader,'scl_slope')||isfield(niftiHeader,'scl_inter')
    slope = 1;
    inter = 0;
    if isfield(niftiHeader,'scl_slope')
...
fwrite(fid, niftiHeader.bvalue, 'float'); %dim
