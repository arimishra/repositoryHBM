...
if(~exist(fullFilename,'file') && ~exist(fullFilename,'dir'))
    error('Could not find specified file or directory.')
end
...
% FDF Directory
elseif(isFDFDir)
    IM = readfdfdirectory(fullFilename);
...
% Nifti file (should be)
if strcmp(deblank(nifti.magic),'n+1')
    
    % skip to data
    fseek(fid, nifti.vox_offset, 'bof');
...
im = reshape(im(1:prod(dims)), dims);
...
while isempty(findstr('checksum',line))
    ...
    %% Orientation direction directly from fdf files
    % Get Orientation
    if (~isempty(findstr(line, 'dro')))
        n = findstr(line, '=');
        dro = sscanf(line(n + 1:length(line)), '%f');
    end
    if (~isempty(findstr(line, 'dpe')))
        n = findstr(line, '=');
        dpe = sscanf(line(n + 1:length(line)), '%f');
    end
    if (~isempty(findstr(line, 'dsl')))
        n = findstr(line, '=');
        dsl = sscanf(line(n + 1:length(line)), '%f');
    end
...
% Add orientation
if (exist('dro', 'var') && exist('dpe', 'var') && exist('dsl', 'var'))
    parms.direction = [dro dpe dsl];
end
...
% try
    data = freadVAXD(fid2,2*dims,'float');
    data = data(:);
    data = data([1:2:length(data)]) + 1i*data([2:2:length(data)]);
    data = reshape(data,dims);
    fclose(fid2);
% catch
%     data = [];
...
function im = readfdfdirectory(directory)
% returns all images in a fdf directory in one image structure

% Get slice and dynamic information
files = dir([directory '/slice*image*echo*.fdf']);
values = zeros(3,length(files));
for i = 1:length(files)
    values(:,i) = sscanf(files(i).name,'slice%dimage%decho%d.fdf');
end
slices = max(values(1,:));
images = max(values(2,:));
echos = max(values(3,:));

% Use first image for meta information
firstImage = vuOpenImage([directory '/slice001image001echo001.fdf']);

% set up data structure
im = firstImage;
im.Data = zeros([firstImage.Dims(1:2) slices images echos]);
im.Spc = firstImage.Spc;
im.Origin = firstImage.Origin;
im.Orientation = firstImage.Orientation;
im.Parms = firstImage.Parms;

% Read data
for i = 1:slices
    for j = 1:images
        for k = 1:echos
            [image, spc, origin, orient, parms] = readFDF(sprintf('%s/slice%03dimage%03decho%03d.fdf',directory,i,j,k));
            im.Data(:,:,i,j,k) = squeeze(single(image));
        end
    end
end
