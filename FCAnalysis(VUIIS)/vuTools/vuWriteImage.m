...
if (isfield(IM,'Parms')&&isfield(IM.Parms,'srow_x')&&isfield(IM.Parms,'aux_file')&&isfield(IM.Parms,'scl_slope'))
    ...
    niftiHeader.bitpix = bitpixN; %bitpix
...
pad = zeros(1, 4-length(niftiHeader.magic));
niftiHeader.magic = [niftiHeader.magic  char(pad)];
fwrite(fid, niftiHeader.magic(1:4), 'uint8'); %Magic string
if (niftiHeader.vox_offset < 348)
    error('MATLAB:vuWriteImage:VoxOffset','Vox_offset must be greater than 348.');
    fclose(fid)
elseif (niftiHeader.vox_offset > 348)
    for i = 349:niftiHeader.vox_offset
