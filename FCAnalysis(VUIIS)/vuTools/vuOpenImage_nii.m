function [IM,pathname,filename] = vuOpenImage_nii(filename, varargin)
...
if(isMed)
    ...
    elseif (regexpi('fid',ext))
        ...
        [im, dims, spc, origin, orient, parms] = readFID(fullFilename);
        IM.Data = squeeze(single(im));
        IM.Dims = dims;
        IM.Spc = spc;
        IM.Origin = origin;
    ...
    elseif (regexpi('spar',ext))
        ...
        [data, span, origin, orient] = readSPAR(fullFilename);
        IM.Data = data;
        IM.Dims = size(IM.Data);
        IM.Span = span;
        IM.Origin = origin;
    ...
    elseif (regexpi('.hdr',ext))
        ...
        if (x(1)=='#')
            ...
            [data, spc, origin, orient] = readHDR(fullFilename);
            IM.Data = data;
            IM.Dims = size(IM.Data);
            IM.Spc = spc;
            IM.Origin = origin;
...
orient = [nifti.srow_x(1:3)';nifti.srow_y(1:3)';nifti.srow_z(1:3)'];
origin = [nifti.srow_x(4) nifti.srow_y(4) nifti.srow_z(4)];
...
% origin
origin = [xoff yoff zoff].*spc - dims.*spc;
origin = origin + spc./2;
...
function [data, spc, origin, orient] = readHDR(fullFilename)
...
% origin
origin = translation./10 - (dims.*spc)/2;
origin = origin + spc./2;
...
function [data, span, origin, orient] = readSPAR(fullFilename)
% SPAR Reader
% By: Kevin Wilson
% Date: March 29, 2007
% Copyright (c) 2007 - Vanderbilt University Institute of Imaging Science
...
% Span
span = [apspan ccspan lrspan]; 

% origin
origin = [-apoff -ccoff lroff]-(span./2);

% Orientation
xrot = [1 0 0;0 cosd(apori) -sind(apori);0 sind(apori) cosd(apori)];
yrot = [cosd(ccori) 0 sind(ccori);0 1 0;-sind(ccori) 0 cosd(ccori)];
zrot = [cosd(lrori) -sind(lrori) 0;sind(lrori) cosd(lrori) 0;0 0 1];
...
if (fid2 == -1)
    ...
    fid2 = fopen(sdatFile,'r','vaxd');
    if (fid2 == -1)
        error('Could not find sdat file');
    end
end
data = fread(fid2,dims,'double');
fclose(fid2);
return;
...
function [data, dims, spc, origin, orient, parms] = readFID(fullFilename)
% FID Reader
...
if (exist(procparFile,'file'))
    ...
    % Origin (Assume phase encode shift zero)
    origin = [0 pp.pro pp.pss(1)] - [fov(1:2)/2 0];
    origin = origin + [spc(1:2)/2 0];
    ...
    pp.psi = pp.psi*-1;
    pp.phi = pp.phi*-1;
    a11 = cosd(pp.psi)*cosd(pp.phi)-cosd(pp.theta)*sind(pp.phi)*sind(pp.psi);
    a12 = cosd(pp.psi)*sind(pp.phi)+cosd(pp.theta)*cosd(pp.phi)*sind(pp.psi);
    a13 = sind(pp.psi)*sind(pp.theta);
    a21 = -1*sind(pp.psi)*cosd(pp.phi) - cosd(pp.theta)*sind(pp.phi)*cosd(pp.psi);
    a22 = -1*sind(pp.psi)*sind(pp.phi) + cosd(pp.theta)*cosd(pp.phi)*cosd(pp.psi);
    a23 = cosd(pp.psi)*sind(pp.theta);
    a31 = sind(pp.theta)*sind(pp.phi);
    a32 = -1*sind(pp.theta)*cosd(pp.phi);
    a33 = cosd(pp.theta);
    orient = [a11 a12 a13;a21 a22 a23; a31 a32 a33];
    orient = orient*[-1 0 0; 0 -1 0; 0 0 1];
    % Flip matrix, because it will be flipped back above in
    % vuRowMajortoColumnMajorSwitch
    orient = permute(orient,[2 1]);
...
if (pp.ne>1)
    if (pp.ns>1)
        data = permute(data,[1 4:length(dims) 3 2]);
        dims = [dims(1) dims(4:end) dims(3) dims(2)];
    else
